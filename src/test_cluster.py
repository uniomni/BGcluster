...
*/20 * * * * monitor /usr/local/bin/test_cluster > /var/tmp/cluster_test_cron.log 2>&1

where it is assumed that /usr/local/bin/test_cluster exists, is executable and has the content
#!/bin/bash

python /etc/bgcluster/src/test_cluster.py $@
...
errlogfile = '/var/tmp/test_cluster.log'  # Has to be writable to all
...
# Run tests
failed = []  # Keep track of failed nodes
